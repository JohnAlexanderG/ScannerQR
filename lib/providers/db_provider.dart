...
export 'package:scannerqr/models/scan_model.dart';
...
class DBProvider {
  ...
  Future<Database> initDB() async {
    ...
    final path = join(documentsDirectory.path, 'ScannerDB');
    print(path);
  ...
  Future<int> nuevoScanRaw(ScanModel nuevoScan) async {
    ...
    final res = await db.rawInsert('''
      INSERT INTO Scanner(id, tipo, valor)
  ...
  Future<int> nuevoScan(ScanModel nuevoScan) async {
    ...
    final db = await database;
    // ...
    final res = await db.insert('Scanner', nuevoScan.toJson());
  ...
  Future<List<ScanModel>> getScansByType(String tipo) async {
    ...
    final res = await db.rawQuery('''
      SELECT * FROM Scanner WHERE tipo = '$tipo'
    ''');
    return res.isNotEmpty ? res.map((s) => ScanModel.fromJson(s)).toList() : [];
  }
  ...
  Future<int> updateScan(ScanModel nuevoScan) async {
    final db = await database;
    final res = await db.update('Scanner', nuevoScan.toJson(),
